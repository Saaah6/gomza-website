...
files.forEach(file => {
  ...
  // Use regex to replace the entire block starting from `if (slide.adType === 'google-search') {`
  // down to `} else {\n          // Default Social Media / Visual style`
  
  const regex = /if\s*\(slide\.adType\s*===\s*'google-search'\)\s*\{[\s\S]*?(?=\}\s*else\s*\{\s*\/\/\s*Default Social Media)/;
